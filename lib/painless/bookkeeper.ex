  ...
  def list_recurring_entries_for_active_tenancies() do
    query =
      from e in Entry,
        ...
        where:
          fragment(
            "? = make_date(date_part('year', now())::int, date_part('month', now())::int, ?)",
            e.transaction_date,
            t.rent_day_of_month
          ),
        select: %{
    ...
    query =
      from t in Tenancy,
        ...
        select: %{
          ...
          description: fragment("? || ' ' || trim(to_char(now(), 'Month'))", t.recurring_description),
          amount: type(t.rent * -1, Money.Ecto.Amount.Type)
        }
